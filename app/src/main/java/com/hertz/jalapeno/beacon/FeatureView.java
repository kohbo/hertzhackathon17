    ...
    public ArchitectView.SensorAccuracyChangeListener getSensorAccuracyListener() {
        return new ArchitectView.SensorAccuracyChangeListener() {
            ...
            public void onCompassAccuracyChanged( int accuracy ) {
                ...
                if ( accuracy < SensorManager.SENSOR_STATUS_ACCURACY_MEDIUM && FeatureView.this != null && !FeatureView.this.isFinishing() && System.currentTimeMillis() - FeatureView.this.lastCalibrationToastShownTimeMillis > 5 * 1000) {
                    Toast.makeText( FeatureView.this, R.string.compass_accuracy_low, Toast.LENGTH_LONG ).show();
                    FeatureView.this.lastCalibrationToastShownTimeMillis = System.currentTimeMillis();
                }
            }
        };
    }
        ...
        switch (requestCode) {
            case WIKITUDE_PERMISSIONS_REQUEST_EXTERNAL_STORAGE: {
                if ( grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED ) {
                    this.saveScreenCaptureToExternalStorage(FeatureView.this.screenCapture);
                } else {
    ...
    protected void saveScreenCaptureToExternalStorage(Bitmap screenCapture) {
        if ( screenCapture != null ) {
            ...
            try {
                ...
                FeatureView.this.startActivity(Intent.createChooser(share, chooserTitle));

            } catch (final Exception e) {
                // should not occur when all permissions are set
                FeatureView.this.runOnUiThread(new Runnable() {

                    @Override
                    public void run() {
                        // show toast message in case something went wrong
                        Toast.makeText(FeatureView.this, "Unexpected error, " + e, Toast.LENGTH_LONG).show();
                    }
                });
            }
        }
